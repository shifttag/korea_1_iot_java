...
//			System.out.println("외부 클래스 필드에 접근(인스턴스) : " + outerField); 
	...
	void outerMethod() {
		...
		class MethodClass{
			void display() {
				System.out.println("지역 변수에 접근: " + localVar);
			}
		}
...
public class A_Inner01 {
	public static void main(String[] args) {
		...
		staticInner.display();	// 외부 클래스 필드에 접근(정적) : 외부 클래스 스태틱 필드
		
		// === 메서드 내부 클래스(지역 클래스) === 
		OuterClass outer2 = new OuterClass();
		outer2.outerMethod();	// 지역 변수에 접근: 로컬 변수
		...
		};	// 추상 클래스를 구현하는 익명 클래스 >> 재사용이 불가, 디버깅이 어려움
