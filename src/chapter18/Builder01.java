...
// 3) 선택적 송성 설정
// : 체이닝 방식으로 선택적 송성을 설정할 수 있는 메서드 추가
...
//		System.out.println(product1.getName());	// 아이폰 16
		...
		Product pd1 = new Product.Builder("아이폰 16", 160).build();
		
		// b) 선택적 속성을 설정한 Product 객체 생성
		Product pd2 = new Product.Builder("갤럭시 S24", 140)
				.description("갤럭시 핸드폰")
				.manufacturer("삼성")
				.build();	// 객체 생성 메서드를 반드시 호출 (최종적으로 Product 반환)
		
		Product pd3 = new Product.Builder("갤럭시 북4", 120)
				.description("갤럭시 노트북")
				.build();	// 객체 생성 메서드를 반드시 호출 (최종적으로 Product 반환)
